...
void setup_uart()
{
    ...
    ESP_ERROR_CHECK(uart_driver_install(UART_NUM_2, RX_BUF * 2, TX_BUF * 2, 0, NULL, intr_alloc_flags));
...
int uart_send(void *data)
{
    ...
    {
        uart_write_bytes(UART_NUM_2, d->ds, d->ds_size);
        ESP_ERROR_CHECK(uart_wait_tx_done(UART_NUM_2, 100));
        int code = uart_read_bytes(UART_NUM_2, d->dr, (RX_BUF - 1), 100 / portTICK_PERIOD_MS);
        ...
        if(code > -1)
        {
            ...
            if(crc_dr == crc_checker && d->dr[0] == d->ds[0])
            {
...
UARTData *uart_data_as_abnt()
{
    ...
    data->ds = (uint8_t *)calloc(66, sizeof(uint8_t));
    data->ds_size = 66;
    data->dr = (uint8_t *)calloc(RX_BUF, sizeof(uint8_t));
    data->dr_size = 0;
...
void set_abnt_data(UARTData *data, uint8_t command, uint8_t *payload, uint8_t payload_len)
{
    ...
    //free(data->dr);
    // data->dr = calloc(RX_BUF, sizeof(uint8_t));
    for(int i = 0; i < RX_BUF; i++)
    {
        data->dr[i] = 0;
    }
    ...
    uint16_t a = crc16arc_bit(0, data->ds, 64);
